...
setwd
getwd()
setwd("/Users/andreadioni/ReCOGsitory")
getwd()
```{r}
getwd()
```
...
```{r}
Listoffiles <- list.files(path = "/users/andreadioni/ReCOGsitory/data", pattern = NULL, full.names = TRUE)
```
...
```{r}
list_files1 <- list.files(path = "/users/andreadioni/ReCOGsitory/data", pattern = NULL, all.files = FALSE,
...
```{r}
Sarah_data <- map_df(Listoffiles,function(x) read.CLAN.file(x))
#dplyr::select()
```
...
```{R}
Sarah_data1 <- select(Sarah_data, Speaker, Gloss, Date)
```
...
```{R}
?write.csv
write.csv(Sarah_data1, "sarahdata1.csv")
```
...
Sarah_data1$Speaker <- as.factor(Sarah_data1$Speaker)
Sarah_data1$Date <- anytime::anydate(Sarah_data1$Date) 

class(Sarah_data1$Speaker) #now these are the right classes
class(Sarah_data1$Gloss)
class(Sarah_data1$Date)
```
...
```{R}
library(dplyr)
MOTCHI_ONLY <- filter(Sarah_data1, Speaker == "MOT" | Speaker == "CHI")
MOTCHI_ONLY
...
```{R}
MOTCHI_ONLY$clean = str_replace_all(MOTCHI_ONLY$Gloss, "[:punct:]", "" )
```
...
MOTCHI_ONLY$words = stri_count_words(MOTCHI_ONLY$Gloss)
```
...
```{R}
library(tidyverse)
groupeddata <- group_by(MOTCHI_ONLY, Speaker, Date) 
```
...
mluDF <- summarise(groupeddata, word_sum =  sum(words),
                                         utterances =  n(),
